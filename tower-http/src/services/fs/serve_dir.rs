...
impl ServeDir {
    ...
    /// If path.file.ext and path.file.ext.gz(.br) coexists retrieve compressed file instend.
    ///
    /// This is will speed up file transfering.
    ///
    /// gzip/brotli file can be generated by gzipper:
    /// gzipper c --exclude jpg,png,ico --threshold 10240 public
    ///
    /// Defaults to `true`.
    pub fn lookup_compressed_file(mut self, is_lookup: bool) -> Self {
        self.lookup_compressed_file = is_lookup;
        self
    }
...
impl<ReqBody> Service<Request<ReqBody>> for ServeDir {
    ...
    fn call(&mut self, req: Request<ReqBody>) -> Self::Future {
        ...
        let is_lookup = self.lookup_compressed_file;
        ...
        let open_file_future = Box::pin(async move {
            ...
            let (full_path, content_encoding) = if is_lookup {
                lookup_compressed_file(full_path, accept_encoding)
            } else {
                (full_path, None)
            };

            let file = File::open(full_path).await?;
            let cache_control = max_age.map(|max_age| {
...
fn lookup_compressed_file(
    full_path: PathBuf,
    accept_encoding: Option<HeaderValue>,
) -> (PathBuf, Option<HeaderValue>) {
    fn check_compressed_file(
        ...
        let mut alt_full_path = full_path.clone();
        ...
        alt_full_path.set_extension(ext);
        // println!("{:?} {:?}", alt_full_path, content_encoding);
        if !alt_full_path.exists() {
            alt_full_path = full_path;
        } else {
            content_encoding = Some(encoding)
        }
        (alt_full_path, content_encoding)
    }
    let alt_full_path = full_path;
    let (alt_full_path, content_encoding) = if let Some(accept_encoding) = accept_encoding {
        let accept_encoding = accept_encoding.to_str().unwrap_or_default();
        let ext = alt_full_path
            .extension()
            .unwrap_or_default()
            .to_str()
            .unwrap()
            .to_owned();
        let (alt_full_path, content_encoding) = if accept_encoding.contains("br") {
        ...
        let (alt_full_path, content_encoding) =
            if content_encoding.is_none() && accept_encoding.contains("gzip") {
                check_compressed_file(alt_full_path, ext, "gz", HeaderValue::from_static("gzip"))
            } else {
                (alt_full_path, None)
            };
        (alt_full_path, content_encoding)
    ...
    fn poll(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        match &mut self.inner {
            Inner::Valid(open_file_future) => {
                let (file, content_type, buf_chunk_size, cache_control, compress_encoding) =
                    match ready!(Pin::new(open_file_future).poll(cx)) {
                        Ok(Output::File {
                            ...
                            buf_chunk_size,
                            cache_control,
                            content_encoding: compress_encoding,
                        }) => (
                            file,
                            content_type,
                            buf_chunk_size,
                ...
                if let Some(compress_encoding) = compress_encoding {
                    res.headers_mut()
                        .insert(header::CONTENT_ENCODING, compress_encoding);
                }
