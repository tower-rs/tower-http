    ...
    if let Some((start, end)) = split_exactly_once_ch(range, '-') {
        if !start.is_empty() {
            if let Some(start) = strict_parse_u64(start) {
                if !end.is_empty() {
                    if let Some(end) = strict_parse_u64(end) {
                        Ok(SyntacticallyCorrectRange::new(
            ...
            if !end.is_empty() {
                if let Some(end) = strict_parse_u64(end) {
                    if end == 0 {
                        invalid!(format!("Range: {} is not satisfiable, suffixed number of bytes to retrieve is zero.", range))
                    } else {
                        Ok(SyntacticallyCorrectRange::new(
                    ...
                    invalid!(format!(
                        "Range: {} is not acceptable, end of range unpareable",
                        range
                    ))
                }
            } else {
                invalid!(format!(
                    "Range: {} is not acceptable, both start of range and end of range are empty",
                    range
                ))
            }
        }
...
fn strict_parse_u64(s: &str) -> Option<u64> {
    if s.starts_with(|ch: char| ch.is_digit(10)) && (s.len() == 1 || !s.starts_with('0')) {
